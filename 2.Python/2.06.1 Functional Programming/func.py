...
def filter_more_than_three(l: Iterable) -> List:
    """ Вернуть строки длинной более трех символов """
    if l is None:
        return []
...
def get_list(int_as_str: str) -> List:
    """ Преобразовать строку с числами в список  """
    if int_as_str is None:
        return []
    if len(int_as_str) == 0:
        return []
    if not isinstance(int_as_str, str):
        return []
    return list(map(int, filter(lambda i: is_int(i), int_as_str.split(" "))))

def sort_func(f, *args, **kwargs) -> List:
    """ Отсортировать результать функции f """
